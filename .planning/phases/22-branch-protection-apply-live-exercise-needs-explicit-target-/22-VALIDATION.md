...
**Executed 2026-09-16 (22-06 Task 2):** both have run — `22-poll-merge-state.sh` with its stale path observed exiting 1 (22-01 Task 2), and `22-evidence/exit2-fixture.json` (22-02 Task 2) — so `wave_0_complete` is now `true` and all four Wave 0 items are ticked. Every row's Status is set from what its plan's SUMMARY recorded; all fourteen are green. The phase gate `diff 22-evidence/rules-before.txt 22-evidence/rules-restored.txt` is empty, re-run at 22-06 against a fresh live read of `rules/branches/main`, and `bash scripts/check-adoption-guide.sh` reports 15 passed / 0 failed after the section 8 update.
