...
| 19-06-01 | 06 | 6 | VAL-01 | T-19-26, T-19-27 | Clean branch cut from `origin/main`; exactly one non-fixture path changed; both new fixtures absent from the branch | cli-assert | `git diff origin/main --name-only`; `git diff origin/main -- fixtures/ .github/ scripts/ --stat` empty; `test ! -e fixtures/vulnerable.py` | ✅ | ⬜ pending |
...
| 19-07-01 | D-10 merge-vs-close decision | VAL-01 | Irreversible action on a shared live public repository; CONTEXT explicitly leaves the choice to execution time; D-04 constrains "close" to require a named follow-up owner. | Claude presents the four captured criteria, the PR's live state, and both options with consequences. Reply `merge` or `close` — and if `close`, name who lands the fixtures on `main`. |
